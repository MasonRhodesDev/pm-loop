...
| 3 | 983 gh reads + 118 merge turns + conflict rounds | **Scripted checklist + merge** — `pm premerge` (open/base/tip==remote/verdict-at-tip/checks/mergeable/role line/closing keywords/trailers) → `pm merge` squash-merges as the App with the role line; refuses on any failed row. | `pmloop/premerge.py`, mirrors the MCP `merge` tool's rule. | PM: `pm premerge` then `pm merge`. Never `gh pr merge`. |
| 4 | STATUS + fact-check lanes ($466, ~3,250 gh calls) | **STATUS from facts** — `pm status-entry` renders a record from `gh pr view --json` + check-runs (+ `git show --numstat`); one prose sentence from the local model, template fallback. **Fact-check by script** — `pm factcheck <repo> <file>` resolves every `#N`, `` `sha` ``, run id and `"quoted string"` (against `--quotes-from` sources). | `pmloop/status.py`, `pmloop/factcheck.py`, `pmloop/llm.py`. | PM after each merge: entry → factcheck → commit as the bot → STATUS PR labelled `no-review`. |
...
`merge.{method,required_check,base,forbid_trailers,closing_keywords_only_in}` · `status.{file,prose,max_prose_words,forbid_patterns}` ·
